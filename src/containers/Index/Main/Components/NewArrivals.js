...
function NewArrivals() {
  ...
  useEffect(() => {
    console.log(product); // Kiểm tra dữ liệu đã được lưu vào Redux Store chưa
  }, [product]);

  useEffect(() => {
    const handleQuickViewClick = (productItem) => {
    ...
    quickViewButtons.forEach((button, index) => {
      button.addEventListener('click', () => handleQuickViewClick(product[index]));
    });
        ...
        <div className="container">
          <div className="row karl-new-arrivals">
            {currentProducts.map((productItem) => (
              <div key={productItem.id} className="col-12 col-sm-6 col-md-4 single_gallery_item women wow fadeInUpBig" data-wow-delay="0.2s">
                <div className="product-img">
                  <img src={productItem.image_path} alt="" />
                  <div className="product-quicview">
                    <a href="#" data-target={`#quickview-${productItem.id}`}>
          ...
          <ul className="pagination justify-content-center" >
            {Array.from({ length: Math.ceil(product.length / productsPerPage) }).map((_, index) => (
              <li key={index} className={`page-item ${index + 1 === currentPage ? 'active' : ''}`}>
                <p onClick={() => paginate(index + 1)} className="page-link" >
                  {index + 1}
                </p>
              </li>
            ))}
